...
export default function MorningBriefCard({ context, revenueAtRisk, needsAction }: Props) {
  ...
  if (needsAction > 0 && revenueAtRisk > 200) attentionLabel = `${revenueAtRisk.toFixed(0)} TND exposés — ${needsAction} commandes nécessitent votre attention`
  else if (needsAction > 0) attentionLabel = `${needsAction} commandes en attente de confirmation`
  else if (revenueAtRisk > 200) attentionLabel = `${revenueAtRisk.toFixed(0)} TND actuellement exposés`
  ...
  return (
    <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] overflow-hidden">
      <div className="px-panel pt-panel pb-3 border-b border-[var(--border)]">
        ...
        <h2 className="text-display text-[var(--text-primary)] mt-1">Brief du matin</h2>
      </div>
      ...
      <div className="p-panel space-y-3">
        ...
        {hasData && rhythm.confirmationSpeedTrend !== "stable" && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] text-[var(--text-secondary)]">
              Confirmations {rhythm.confirmationSpeedTrend === "improving" ? "plus rapides" : "plus lentes"}
            </span>
          </div>
        )}
        ...
        {continuity.length > 0 && (
          <div className="pt-2 border-t border-[var(--border)] space-y-1.5">
            <p className="text-[10px] text-[var(--text-tertiary)] uppercase tracking-wider">Tendances</p>
        ...
        {rhythm.highRiskPeriods.length > 0 && (
          <div className="pt-2 border-t border-[var(--border)]">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-[10px] text-[var(--text-tertiary)] uppercase tracking-wider mb-1">Périodes à risque</p>
