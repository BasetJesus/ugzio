...
import BuyerConfirmCelebration from "./BuyerConfirmCelebration"
...
export default function BuyerConfirmationButton({ token, orderId, sellerName, sellerPhone }: Props) {
  ...
  const handleConfirm = useCallback(async () => {
    setStatus("loading")
    try {
      const res = await fetch("/api/v1/buyer/confirm", {
        method: "POST",
  ...
  return (
    <>
      <BuyerConfirmCelebration
        sellerName={sellerName}
        open={celebrationOpen}
        onComplete={() => setCelebrationOpen(false)}
      />

      {status === "confirmed" && !celebrationOpen && (
