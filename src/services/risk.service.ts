...
// ─── Mock Risk Generation (for dev/seed data) ───

export function generateMockRisk(input: {
  isFirstTime?: boolean;
  amount?: number;
  buyerWilaya?: string | null;
  status?: string;
}): { riskScore: number; trustScore: number; riskLevel: RiskLevel; signals: string[] } {
  let score = 50;
  ...
  if (input.amount && input.amount > 150) {
    score += 10;
    signals.push("high-amount");
  }

  const RISKY_WILAYAS = ["Kairouan", "Gabès", "Kasserine"];
  if (input.buyerWilaya && RISKY_WILAYAS.includes(input.buyerWilaya)) {
    score += 10;
    signals.push("unusual-region");
  }

  if (input.status === "PENDING_RESCHEDULE") {
    score += 15;
  ...
  if (input.status === "REFUSED") {
    score += 20;
    signals.push("payment-irregularity");
  }

  if (input.status === "INTELLIGENT_CANCEL") {
    score += 10;
